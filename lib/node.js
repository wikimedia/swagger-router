...
class Node {
    ...
    getChild(segment, params) {
        if (segment.constructor === String) {
            // Fast path
            let res = this._children[this._keyPrefix + segment];
            if (!res && segment !== '') {
    ...
    keys() {
        if (this._children['*'] || this._children['**']) {
            ...
            Object.keys(this._children).forEach((key) => {
                ...
                // /double//slash)
                if (key !== this._keyPrefix || this._children[key].hasChildren()) {
                    res.push(key.replace(this._keyPrefixRegExp, ''));
                }
            });
            return res.sort();
        }
