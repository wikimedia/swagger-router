...
function createURIResolver(uri, globals) {
    // Check if this is a simple path template of the form
    // /{domain}/foo/{bar}/, with each path segment either a plain string, or
    // a simple variable reference without dots, function calls etc. No path
    // segment can be a mix of a static string & a variable substitution.
    if (/^(?:\/(?:[a-zA-Z_-]+|\{[\/\+]?[a-zA-Z_-]+\}))*\/?$/.test(uri)) {
...
function Template(spec, globalsInit) {
    ...
    // console.log(completeTAssemblyTemplate);
    var res;
    var callback = function(bit) {
        if (res === undefined) {
            res = bit;
